...
class RoomService extends BaseService {
  ...
  Future<List<Room>> getRoomsByHotelId(int hotelId) async {
    ...
    final response = await authenticatedGet('room/get-all-rooms?hotelId=$hotelId'); // Updated Endpoint

    if (response.statusCode == 200) {
      final List<dynamic> jsonData = json.decode(response.body);
      return jsonData.map((json) => Room.fromJson(json)).toList();
    } else {
      throw Exception('Failed to load rooms for hotel $hotelId: ${response.statusCode} ${response.body}');
    }
  }

  // Get a specific room by its ID
  Future<Room?> getRoomById(int roomId) async {
    final response = await authenticatedGet('room/get-room-by-id?id=$roomId'); // Updated Endpoint

    if (response.statusCode == 200) {
      final Map<String, dynamic> jsonData = json.decode(response.body);
      if (jsonData.isEmpty) return null; // Or handle as appropriate if API returns empty for not found
      return Room.fromJson(jsonData);
    } else if (response.statusCode == 404) {
      return null; // Room not found
    } else {
      throw Exception('Failed to load room $roomId: ${response.statusCode} ${response.body}');
    }
  }
  ...
  Future<Room> updateRoomState(int roomId, String newState) async {
    ...
    final Map<String, dynamic> body = {
      'roomId': roomId, // This field name is an assumption for UpdateRoomStateResource
      'state': newState,
    };
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to update room state for $roomId: ${response.statusCode} ${response.body}');
    }
  }
  ...
  Future<void> deleteRoom(int roomId) async {
    ...
    if (response.statusCode != 204 && response.statusCode != 200) { // 204 No Content is also a success
      throw Exception('Failed to delete room $roomId: ${response.statusCode} ${response.body}');
    }
  }
}
